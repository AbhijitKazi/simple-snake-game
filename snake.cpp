...
void clearScreen(){
    ...
    if (!FillConsoleOutputCharacter(
        ...
        homeCoords,                     //Coordinates of the first cell
        &count                          //receives the number of characters writtenz
                                    )) return;
...
void snake::updateSnake(int delay){
    ...
    body[0].x = prev[0].x + dx[direction];          //Head of the snake
...
void userInput( void* id){
    do{
        ...
        switch(c){
            ...
            case 'E':case 'e': input = EXIT; break;
        }
...
int main()
{
    int delay;
    char speed;
    cout<<"Please select a Speed Setting: ";
    cout<<"\n'S' for Slow";
    cout<<"\n'M' for Moderate";
    cout<<"\n'F' for Fast";
    cout<<"\n : ";
    ...
    switch(speed){
        case 'S':case 's': delay = 200; break;
        case 'M':case 'm':delay = 130;break;
        case 'F':case 'f':delay = 65;break;
        default: delay = 200;
    }
    srand(time(NULL));
    snake Snake;
    ...
    cout<<"Your score is "<<Snake.getFoodCounter()-1<<" !"<<endl;
    getch();
    return 0;
}
